...
func CreateAmbulance(c *gin.Context) {
	...
	//  ค้นหา user ด้วย id
	if tx := entity.DB().Where("id = ?", ambulance.RecorderID).First(&recorder); tx.RowsAffected == 0 {
	...
	wv := entity.Ambulance{
		...
		Recorder:      recorder,
		Recording:     ambulance.Recording,    // ตั้งค่าฟิลด์ REcording
...
func GetAmbulance(c *gin.Context) {
	var ambulance entity.Ambulance
	id := c.Param("id")
	if err := entity.DB().Preload("AmbulanceType").Preload("Brand").Preload("Status").Preload("Recorder").Raw("SELECT * FROM ambulances WHERE id = ?", id).Find(&ambulance).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListAmbulance(c *gin.Context) {
	...
	if err := entity.DB().Preload("AmbulanceType").Preload("Brand").Preload("Status").Preload("Recorder").Raw("SELECT * FROM ambulances").Find(&ambulances).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
