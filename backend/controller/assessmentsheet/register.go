...
func CreateRegister(c *gin.Context) {
    ...
    // 9: ค้นหา notify ด้วย id
    if tx := entity.DB().Where("id = ?", register.NotifyID).First(&notify); tx.RowsAffected == 0 {
    ...
    rt := entity.Register{
    	...
    	Notify:             	notify,               	// โยงความสัมพันธ์กับ Entity Notify
...
func GetRegister(c *gin.Context) {
	...
	if err := entity.DB().Preload("AssessmentSheet").Preload("Ambulance").Preload("Notify").Raw("SELECT * FROM registers WHERE id = ?", id).Find(&register).Error; err != nil {
...
func ListRegisters(c *gin.Context) {
	...
	if err := entity.DB().Preload("AssessmentSheet").Preload("Ambulance").Preload("Notify").Raw("SELECT * FROM registers").Find(&registers).Error; err != nil {
